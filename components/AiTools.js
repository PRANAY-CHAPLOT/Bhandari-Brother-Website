...
import { searchKnowledgeBase, locateSuppliers, generateIndustrialPrototype, editIndustrialImage, fileToGenerativePart } from '../services/geminiService.js';
import { AI_MODE } from '../types.js';
import { html } from 'htm/react';

const AiTools = () => {
  const [activeMode, setActiveMode] = useState(AI_MODE.SEARCH);
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <div className="flex-grow">
            ...
            <div className="flex gap-2">
                ...
                ${activeMode === AI_MODE.GENERATE && html`
                    <select 
                        value=${genSize} 
                ...
                >
                    ${loading ? html`<${Loader2} className="animate-spin" />` : 'Execute'}
                </button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-50 rounded-2xl p-6 min-h-[200px] border border-slate-100">
            ...
            ` : html`
                <div className="space-y-6">
                    ...
                    ${groundingSources.length > 0 && html`
                                    ...
                                    return html`
                                        ...
                                        >
                                            <div className=${`p-2 rounded-full ${isMap ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600'}`}>
                                                ${isMap ? html`<${MapPin} size=${16} />` : html`<${ExternalLink} size=${16} />`}
                                            </div>
