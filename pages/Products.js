    ...
    <div className="min-h-screen pt-28 pb-20 bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="mb-12 animate-slide-up flex flex-col md:flex-row justify-between items-end gap-6">
          ...
          ${(selectedCategories.length > 0 || selectedBrands.length > 0) && html`
              <button 
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className=${`w-4 h-4 rounded border flex items-center justify-center transition-colors ${isSelected ? 'bg-[#115FAD] border-[#115FAD]' : 'border-slate-300 dark:border-slate-600'}`}>
                                                ${isSelected && html`<${Check} size=${10} className="text-white" />`}
                                            </div>
                                            ${cat}
                                        </div>
                ...
                <div>
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                        ${categories.map(cat => html`
                            ...
                            >
                                ${cat}
                                ${selectedCategories.includes(cat) && html`<${X} size=${14} className="opacity-70" />`}
                            </button>
                        `)}
                    </div>
                </div>
                ...
                <div>
                     ...
                     <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                        ${brands.map(brand => html`
                            ...
                            >
                                ${brand}
                                ${selectedBrands.includes(brand) && html`<${X} size=${14} className="opacity-70" />`}
                            </button>
                        `)}
                    </div>
                </div>
            ...
            <div className="lg:col-span-3">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ${filteredProducts.length > 0 ? (
                        filteredProducts.map((product, idx) => html`
                            <div 
                                key=${product.id} 
                                    ...
                                    ${product.features && html`
                                        <div className="mb-6 pt-4 border-t border-slate-100 dark:border-slate-800">
                                            <ul className="space-y-1">
                                                ${product.features.slice(0, 2).map((feature, i) => html`
                        ...
                        html`<div className="col-span-full py-20 text-center text-slate-400 flex flex-col items-center">
